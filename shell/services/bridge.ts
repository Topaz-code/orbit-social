...
(function () {
  ...
  var lastSignatureAt = 0;

  var callPattern = /incoming\\s+(voice\\s+|video\\s+)?call|is\\s+calling(\\s+you)?/i;



  var scanForIncomingCall = function (node) {

    if (!node || document.visibilityState === 'visible' && document.hasFocus()) { return; }

    var text = (node.innerText || node.textContent || '').trim();

    if (!text || text.length > 400 || !callPattern.test(text)) { return; }

    var signature = text.replace(/\\s+/g, ' ').slice(0, 120);

    var now = Date.now();

    if (signature === lastSignature && now - lastSignatureAt < 60000) { return; }

    lastSignature = signature; lastSignatureAt = now;

    var isVideo = /video/i.test(text);

    var nameMatch = text.match(/^([^\\n]{2,60}?)\\s+is\\s+calling/im) || text.match(/from\\s+([^\\n]{2,60})/i);

    var heading = node.querySelector && node.querySelector('h1,h2,h3,strong,[data-caller-name]');

    var callerName = (nameMatch && nameMatch[1]) || (heading && heading.textContent) || 'Orbit user';

    var avatarEl = node.querySelector && node.querySelector('img');
    ...
    post('INCOMING_CALL', {
      ...
      callerAvatar: avatarEl ? avatarEl.src : undefined,
    ...
    var observer = new MutationObserver(function (mutations) {
      ...
      for (var i = 0; i < mutations.length; i++) {
        ...
        for (var j = 0; j < added.length; j++) {
          ...
          if (added[j].nodeType === 1) { scanForIncomingCall(added[j]); }
